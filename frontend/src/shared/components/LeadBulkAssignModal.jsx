  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setUsers(userList.map((u) => ({ ...u, role: getUserRole(u) })))
      } catch (error) {
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[10000] p-4">
      <div className={`${isLight ? 'bg-white text-slate-800' : 'bg-slate-900 text-white'} w-full max-w-md rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]`}>
        ...
        <div className="p-4 flex-1 overflow-y-auto space-y-4">
          {errorMessage ? (
            <div id="reassign-lead-error-banner" className={`rounded-2xl border px-4 py-3 shadow-sm ${isLight ? 'border-red-200 bg-red-50 text-red-800' : 'border-red-500/30 bg-red-500/10 text-red-100'}`}>
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto pr-1 custom-scrollbar">
            {loading ? (
              <div className="text-center py-4 text-sm text-gray-500">{isArabic ? 'جارٍ التحميل...' : 'Loading...'}</div>
              ...
              >
                ...
                <div className="w-8 h-8 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden">
                  {user.avatar ? <img src={user.avatar} alt={user.name} className="w-full h-full object-cover" /> : <FaUser className="text-gray-400 text-xs" />}
                </div>
          ...
          <div>
            ...
            <div className={`grid grid-cols-2 p-1 rounded-xl border ${isLight ? 'bg-gray-50 border-gray-200' : 'bg-slate-800 border-slate-700'}`}>
              <button
                ...
                className={`text-sm py-1.5 rounded-lg transition-all ${assignMethod === 'fresh' ? 'bg-white text-slate-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
              >
                {isArabic ? 'جديد' : 'New'}
              </button>
              <button
                ...
                if (!canAssignAsManager) {
                  return (
                    <div className={`grid grid-cols-1 p-1 rounded-xl border ${isLight ? 'bg-gray-50 border-gray-200' : 'bg-slate-800 border-slate-700'}`}>
                      <button onClick={() => setAssignRole('sales')} className="text-sm py-1.5 rounded-lg transition-all bg-white text-slate-900 shadow-sm">
                ...
                return (
                  <div className={`grid grid-cols-2 p-1 rounded-xl border ${isLight ? 'bg-gray-50 border-gray-200' : 'bg-slate-800 border-slate-700'}`}>
                    <button
                      ...
                      className={`text-sm py-1.5 rounded-lg transition-all ${assignRole === 'sales' ? 'bg-white text-slate-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
                      {isArabic ? 'كمسؤول مبيعات' : 'As Sales Person'}
                    </button>
                    <button
                      onClick={() => setAssignRole('manager')}
                      className={`text-sm py-1.5 rounded-lg transition-all ${assignRole === 'manager' ? 'bg-white text-slate-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}
                    >
        ...
        <div className={`p-4 border-t ${isLight ? 'bg-gray-50 border-gray-100' : 'bg-slate-800/50 border-slate-800'} flex items-center justify-end gap-3`}>
          <button
            onClick={onClose}
            className={`px-4 py-2 text-sm font-medium rounded-lg transition-colors ${isLight ? 'text-slate-600 hover:bg-gray-200 bg-white border border-gray-200' : 'text-slate-300 hover:bg-slate-700 bg-slate-800 border border-slate-700'}`}
          >
          ...
          <button
            ...
            className={`px-6 py-2 text-sm font-medium rounded-lg shadow-lg shadow-blue-500/20 transition-all ${selectedUser && !submitting ? 'bg-blue-600 text-white hover:bg-blue-700 hover:scale-[1.02]' : 'bg-gray-300 text-gray-500 cursor-not-allowed'}`}
          >
