...
export default function Sources() {
  ...
  const handleEdit = (source) => {
    ...
    setFormData({ name: source.name || '', name_ar: source.name_ar || '' })
    setIsModalOpen(true)
  }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (error?.response?.status === 422 && data?.requires_reassignment) {
        ...
        setReassignState({
          sourceId: id,
          sourceName: getSourceDisplayName(source, isArabic) || source?.name || '',
    ...
    <>
      ...
      >
        <div className="space-y-4">
            ...
            <div className="space-y-1.5">
                <label className={`text-sm font-semibold ${isLight ? 'text-black' : 'text-white'}`}>{t('Arabic Name')}</label>
                <input 
                    className={`w-full p-3 rounded-xl border border-gray-200 dark:border-gray-700 bg-gray-800 ${isLight ? 'text-black' : 'text-white'} outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all`}
                    value={formData.name_ar}
                    onChange={e => setFormData({...formData, name_ar: e.target.value})}
                    placeholder={t('Enter Arabic source name')}
                    dir="rtl"
                />
            </div>
        </div>
            ...
            <div className="space-y-1.5">
                ...
                >
                    ...
                    {reassignOptions.map(s => (
                        <option key={s.id} value={s.id}>{getSourceDisplayName(s, isArabic)}</option>
                    ))}
                </select>
