...
import { getSourceCanonicalName, mapSourceToOption } from '../shared/utils/sourceDisplay'
    ...
    if (name === 'provider') {
      ...
      setForm(prev => {
        ...
        if (!prev.source) {
          ...
          if (desired) {
            const match = sources.find(s => normalize(getSourceCanonicalName(s)) === normalize(desired))
            if (match) next.source = getSourceCanonicalName(match)
          }
        }
        return next
      })
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="space-y-1">
            ...
            <SearchableSelect
              ...
              options={sources.map(source => mapSourceToOption(source, isArabic)).filter(Boolean)}
              isRTL={isArabic}
              ...
              <form onSubmit={onSubmit} className={`space-y-4 ${isArabic ? 'text-right' : 'text-left'}`}>
                ...
                <div>
                  ...
                  <SearchableSelect
                    ...
                    options={sources.map(s => mapSourceToOption(s, isArabic)).filter(Boolean)}
                    isRTL={isArabic}
