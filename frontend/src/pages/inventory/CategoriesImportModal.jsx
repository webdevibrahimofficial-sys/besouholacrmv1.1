...
export default function CategoriesImportModal({ isOpen, onClose, onImport }) {
  ...
  const copy = useMemo(() => ({
    title: isRTL ? 'استيراد التصنيفات من ملف Excel' : 'Import Categories from Excel',
    downloadTitle: isRTL ? 'تحميل ملف Excel' : 'Download Excel Template',
    downloadDescription: isRTL
    ...
    importPrepared: (count) => isRTL
      ? `تم تجهيز ${count} تصنيف للاستيراد`
      : `Prepared ${count} categories for import`,
  }), [isRTL])
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      logImportEvent({
        ...
        format: 'xlsx',
        status: 'failed',
        errorMessage: error?.message || 'Category import failed',
        metaData: { reason_code: 'categories_import_failed' },
      })
    } finally {
      setImporting(false)
    }
        ...
        <div className={`px-6 py-6 overflow-y-auto custom-scrollbar ${isDark ? 'bg-[#0f172a]' : 'bg-white'}`}>
          ...
          <div className="space-y-2">
            ...
            <div
              ...
              onClick={() => document.getElementById('categories-excel-file-input')?.click()}
            ...
            >
              ...
              <input
                id="categories-excel-file-input"
                type="file"
                accept=".xlsx,.xls"
                className="hidden"
              ...
              <button
                ...
                onClick={(e) => {
                  e.stopPropagation()
                  document.getElementById('categories-excel-file-input')?.click()
                }}
