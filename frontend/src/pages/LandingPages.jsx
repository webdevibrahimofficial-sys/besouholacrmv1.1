...
import { buildPublicLandingUrl } from '../shared/utils/landingPageUrl'
...
export default function LandingPages() {
  ...
  const fetchLandingPages = async () => {
    try {
      ...
      const mapped = (res.data.data || []).map(item => ({
        ...
        url: item.url || buildPublicLandingUrl({ origin: window.location.origin, slug: item.slug })
      }))
      setRows(mapped)
    } catch (error) {
  ...
  const sourceOptions = useMemo(() => {
    if (sources.length > 0) {
      return sources.map(s => mapSourceToOption(s, isRTL)).filter(Boolean)
    }
    return Array.from(new Set(rows.map(r => r.source))).filter(Boolean).map((value) => ({ value, label: value }))
  }, [sources, rows, isRTL])
