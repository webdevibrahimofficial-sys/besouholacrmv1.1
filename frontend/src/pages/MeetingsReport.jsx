...
export default function MeetingsReport() {
  ...
  const sourceOptions = useMemo(() => {
    return sources
      .map((s) => {
        const value = getSourceCanonicalName(s)
        const label = getSourceDisplayName(s, isRTL) || `#${s.id}`
  ...
  const channelData = useMemo(() => {
    const map = new Map();
    meetings.forEach(m => {
      const source = localizeSourceLabel(m.source || (isRTL ? 'غير معروف' : 'Unknown'));
              ...
              {paginatedMeetings.map((meeting) => (
                <React.Fragment key={meeting.id}>
                  <tr className="hover:bg-white/5 transition-colors">
                    ...
                    <td className={`hidden md:table-cell px-4 py-3 ${isLight ? 'text-black' : 'text-white'} `}>{localizeSourceLabel(meeting.source)}</td>
                    ...
                    <td className="px-4 py-3 flex items-center justify-center gap-2">
                      <button className="p-1.5 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors" title={isRTL ? 'عرض العميل' : 'Preview'} onClick={() => { setSelectedLead({ id: meeting.leadId, name: meeting.leadName, phone: meeting.mobile, source: localizeSourceLabel(meeting.source), project: meeting.project, assigned_to: meeting.assigned_to, sales_person: meeting.salesPerson }); setShowLeadModal(true); }}>
                        <RiEyeLine size={18} />
                      </button>
                      {isAdminOrManager && (
                    ...
                    <tr className="md:hidden bg-white/5">
                      <td colSpan={9} className="px-4 py-3">
                        <div className="grid grid-cols-2 gap-3 text-xs">
                          ...
                          <div className="flex flex-col"><span>{isRTL ? 'المصدر' : 'Source'}</span><span className={`${isLight ? 'text-black' : 'text-white'}`}>{localizeSourceLabel(meeting.source)}</span></div>
