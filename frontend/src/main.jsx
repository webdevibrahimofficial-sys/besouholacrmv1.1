...
try {
  if (typeof window !== 'undefined') {
    ...
    if (isPublicLandingPath && (!hash || hash === '#')) {
...
try {
  if (typeof navigator !== 'undefined' && typeof navigator.share === 'function' && !navigator.__shareWithClipboardFallbackPatched) {
    const nativeShare = navigator.share.bind(navigator)
    navigator.share = async (shareData) => {
      try {
        return await nativeShare(shareData)
      } catch (error) {
        const shareName = String(error?.name || '')
        const shareUrl = typeof shareData?.url === 'string' ? shareData.url : ''
        if ((shareName === 'NotAllowedError' || shareName === 'AbortError') && shareUrl && navigator.clipboard?.writeText) {
          await navigator.clipboard.writeText(shareUrl)
          if (typeof window !== 'undefined') {
            window.dispatchEvent(new CustomEvent('app:toast', {
              detail: {
                type: 'success',
                message: String(i18n.language || '').startsWith('ar')
    ...
    navigator.__shareWithClipboardFallbackPatched = true
  }
} catch {}
