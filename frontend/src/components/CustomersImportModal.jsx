...
const CustomersImportModal = ({ onClose, onImport, isRTL }) => {
  ...
  const generateTemplate = () => {
    ...
    const dummyData = [
        ['C-1001', 'John Doe', '01012345678', 'john@example.com', 'Individual', 'Website', 'Example Corp', '123456789', 'Egypt', 'Cairo', '123 Main St', 'Admin', 'VIP, New', 'Sample note'],
        ['C-1002', 'Jane Smith', '01123456789', 'jane@example.com', 'Company', 'Referral', 'Smith LLC', '987654321', 'Egypt', 'Alexandria', '456 Side Ave', 'Agent 1', 'Lead', 'Another note']
    ]
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = (e) => {
        try {
          ...
          const required = ['customer name', 'phone'] // Minimal requirements
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        const mappedData = jsonData.map(row => {
          return {
            customerCode: findKey(row, ['Customer Code', 'كود العميل', 'Code']),
            name: findKey(row, ['Customer Name', 'اسم العميل', 'Name']),
            phone: findKey(row, ['Phone', 'الهاتف', 'Mobile']),
            email: findKey(row, ['Email', 'البريد الإلكتروني', 'Mail']),
            type: findKey(row, ['Type', 'النوع']) || 'Individual',
            source: findKey(row, ['Source', 'المصدر']) || 'New',
            companyName: findKey(row, ['Company Name', 'اسم الشركة', 'Company']),
            taxNumber: findKey(row, ['Tax Number', 'الرقم الضريبي', 'Tax ID']),
            country: findKey(row, ['Country', 'الدولة']),
            city: findKey(row, ['City', 'المدينة']),
            addressLine: findKey(row, ['Address', 'العنوان']),
            assignedSalesRep: findKey(row, ['Sales Rep', 'مسؤول المبيعات', 'Assigned To']),
            tags: (findKey(row, ['Tags', 'الوسوم']) || '').split(',').map(t => t.trim()).filter(Boolean),
        ...
        logImportEvent({
          module: 'Customers',
          fileName: excelFile?.name || 'customers_import.xlsx',
          format: 'xlsx',
          status: 'success',
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold" style={{ color: isDark ? 'white' : '#111827' }}>{isRTL ? 'استيراد عملاء' : 'Import Customers'}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 overflow-y-auto custom-scrollbar">
          ...
          {importSummary && (
            <div className="mt-4 px-4 py-3 rounded-lg bg-green-50 text-green-700 border border-green-200 dark:bg-green-900/50 dark:text-green-200 dark:border-green-800">
              {isRTL ? `تم استيراد ${importSummary.added} عميل بنجاح` : `Successfully imported ${importSummary.added} customers`}
            </div>
          )}

          <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
            {isRTL ? 'الحقول المدعومة: الاسم، الهاتف، البريد، الشركة، النوع، المصدر...' : 'Supported Fields: Name, Phone, Email, Company, Type, Source...'}
          </div>
        </div>
      </div>
    </div>
