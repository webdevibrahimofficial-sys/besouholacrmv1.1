...
const SalesInvoicesImportModal = ({ onClose, onImport, isRTL }) => {
  ...
  const generateTemplate = () => {
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Sales Invoices Template')
    const fileName = 'sales_invoices_template.xlsx'
  ...
  const executeImport = async () => {
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        logImportEvent({
          module: 'Sales Invoices',
          fileName: excelFile?.name || 'sales_invoices_import.xlsx',
          format: 'xlsx',
          status: 'success',
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold" style={{ color: isDark ? 'white' : '#111827' }}>{isRTL ? 'استيراد فواتير المبيعات' : 'Import Sales Invoices'}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 overflow-y-auto custom-scrollbar">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: isDark ? '#d1d5db' : '#4b5563' }}>
                    {isRTL ? 'استخدم هذا النموذج لإضافة فواتير مبيعات جديدة' : 'Use this template to add new sales invoices'}
                  </p>
                </div>
              </div>
              <button
          ...
          {importSummary && (
            <div className="mt-4 px-4 py-3 rounded-lg bg-green-50 text-green-700 border border-green-200 dark:bg-green-900/50 dark:text-green-200 dark:border-green-800">
              {isRTL ? `تم استيراد ${importSummary.added} فاتورة مبيعات بنجاح` : `Successfully imported ${importSummary.added} sales invoices`}
            </div>
          )}
...
export default SalesInvoicesImportModal
