...
/**
 * Shared Add New Lead form used by the /leads/new page and in-place modals (e.g. WhatsApp Mirror inbox).
 */
export const AddNewLeadForm = ({
  embedded = false,
  initialPrefill = null,
  lockPhone = false,
  allowExtraLeads = true,
  onSuccess = null,
  onCancel = null,
} = {}) => {
  ...
  const isTelesalesMode = !embedded && location.pathname.startsWith('/telesales');
  ...
  const sourceOptions = useMemo(() => {
    const opts = sourcesList
      .map(s => mapSourceToOption(s, isRTL))
      .filter(Boolean);

    const preferred = String(initialPrefill?.source || source || '').trim();
    if (preferred && !opts.some((option) => String(option.value).toLowerCase() === preferred.toLowerCase())) {
      opts.unshift({ value: preferred, label: preferred });
    }

    return opts;
  }, [sourcesList, isRTL, initialPrefill?.source, source]);
  ...
  useEffect(() => {
    ...
    if (initialPrefill.name) setName(String(initialPrefill.name));
    if (initialPrefill.source) setSource(String(initialPrefill.source));
    if (initialPrefill.notes != null) setNote(String(initialPrefill.notes));
    if (initialPrefill.email) setEmail(String(initialPrefill.email));
    if (initialPrefill.company) setCompany(String(initialPrefill.company));
    if (initialPrefill.campaign) setCampaign(String(initialPrefill.campaign));
    if (initialPrefill.country) setCountry(String(initialPrefill.country));

    const phoneValue = String(initialPrefill.phone || '').trim();
    if (phoneValue) {
      const parsed = splitPhoneInput(phoneValue, defaultDialCode || '');
      setMobileNumbers([{
        code: parsed.code || defaultDialCode || '',
        number: String(parsed.number || '').replace(/\D+/g, ''),
      }]);
    }
  }, [COUNTRY_CODES, defaultDialCode, initialPrefill]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Embedded flows (e.g. WhatsApp Mirror) may prefill a source that is not yet in tenant sources.
      if (embedded && source.trim()) {
        try {
          ...
          // Ignore duplicates / permission issues; LeadController will validate existence.
        }
      }
      ...
      const primaryLead = primaryRes?.data?.data || primaryRes?.data || null;

      if (typeof onSuccess === 'function') {
        await onSuccess({
          lead: primaryLead,
          response: primaryRes?.data,
          savedTotal,
          savedDuplicates,
        });
        return;
  ...
  if (!canAddLead) {
    return (
      <div className={`${embedded ? 'p-4' : 'p-6'} bg-[var(--content-bg)] text-[var(--content-text)]`}>
  ...
  return (
    <div className={`${embedded ? 'p-0' : 'p-3 sm:p-6 pb-24'} bg-[var(--content-bg)] text-[var(--content-text)]`}>
      {!embedded && (
      ...
      <div className={`p-4 md:p-6 rounded-lg border ${formTone}`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {!lockPhone && mobileNumbers.slice(1).map((m, idx) => (
              ...
              <div className="mt-6">
                {allowExtraLeads && extraLeads.map((l, i) => (
      ...
      <div className={`${embedded ? 'sticky bottom-0 z-10' : 'sticky bottom-0 left-0 right-0 z-50'} border-t-2 ${isLight ? 'bg-white border-gray-300 shadow-2xl' : 'bg-gray-900 border-gray-600 shadow-2xl'} backdrop-blur-md mt-6`}>
          ...
          <div className="inline-flex w-fit items-center gap-2">
            {embedded && typeof onCancel === 'function' && (
              <button
                type="button"
                onClick={onCancel}
                className={`inline-flex items-center gap-2 px-4 py-2 rounded-md font-semibold border transition ${
                  isLight
