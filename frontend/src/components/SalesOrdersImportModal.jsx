...
const SalesOrdersImportModal = ({ onClose, onImport, isRTL }) => {
  ...
  const generateTemplate = () => {
    ...
    const dummyData = [
        ['C-1001', 'John Doe', 'Draft', 'Q-1001', '2023-10-15', 'Cash', 'Net 15', 'Admin', 'Admin', 'Laptop X1', '1', '1000'],
        ['C-1002', 'Jane Smith', 'Confirmed', 'Q-1002', '2023-10-20', 'Bank Transfer', 'Net 30', 'Agent 1', 'Admin', 'Setup Fee', '1', '500']
    ]
    
    const ws = XLSX.utils.aoa_to_sheet([headers, ...dummyData])
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, 'Sales Orders Template')
    const fileName = 'sales_orders_template.xlsx'
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        const processedData = jsonData.map((row, index) => ({
            id: `SO-IMP-${Date.now()}-${index}`,
            customerCode: findKey(row, ['Customer Code', 'كود العميل']) || '',
            customerName: findKey(row, ['Customer Name', 'اسم العميل']) || 'Unknown',
            status: findKey(row, ['Status', 'الحالة']) || 'Draft',
            quotationId: findKey(row, ['Quotation Code', 'كود عرض السعر']) || '',
            deliveryDate: findKey(row, ['Delivery Date', 'تاريخ التوصيل']) ? new Date(findKey(row, ['Delivery Date', 'تاريخ التوصيل'])).toISOString() : new Date().toISOString(),
            paymentType: findKey(row, ['Payment Type', 'نوع الدفع']) || 'Cash',
            paymentTerms: findKey(row, ['Payment Terms', 'شروط الدفع']) || '',
            salesPerson: findKey(row, ['Sales Person', 'مندوب المبيعات']) || '',
            createdBy: findKey(row, ['Created By', 'أنشئت بواسطة']) || 'System',
            items: findKey(row, ['Item Name', 'اسم الصنف']) ? [{
                id: 1,
        ...
        setImportSummary({ added: processedData.length })
        logImportEvent({
          module: 'Sales Orders',
          fileName: excelFile?.name || 'sales_orders_import.xlsx',
          format: 'xlsx',
          status: 'success',
          meta: { total: processedData.length },
        })
        setImporting(false)
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <h3 className="text-lg font-bold" style={{ color: isDark ? 'white' : '#111827' }}>{isRTL ? 'استيراد أوامر البيع' : 'Import Sales Orders'}</h3>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="px-6 py-6 overflow-y-auto custom-scrollbar">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs" style={{ color: isDark ? '#d1d5db' : '#4b5563' }}>
                    {isRTL ? 'استخدم هذا النموذج لإضافة أوامر بيع جديدة' : 'Use this template to add new sales orders'}
                  </p>
                </div>
              </div>
              <button
          ...
          {importSummary && (
            <div className="mt-4 px-4 py-3 rounded-lg bg-green-50 text-green-700 border border-green-200 dark:bg-green-900/50 dark:text-green-200 dark:border-green-800">
              {isRTL ? `تم استيراد ${importSummary.added} أمر بيع بنجاح` : `Successfully imported ${importSummary.added} sales orders`}
            </div>
          )}

          <div className="mt-3 text-xs" style={{ color: isDark ? '#9ca3af' : '#6b7280' }}>
            {isRTL ? 'الحقول المدعومة: كود العميل، اسم العميل، الحالة، تاريخ التوصيل...' : 'Supported Fields: Customer Code, Customer Name, Status, Delivery Date...'}
          </div>
        </div>
      </div>
    </div>
...
export default SalesOrdersImportModal
