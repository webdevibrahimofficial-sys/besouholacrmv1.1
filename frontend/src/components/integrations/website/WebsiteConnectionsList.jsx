...
export default function WebsiteConnectionsList({
  ...
  if (loading) {
    return <div className="p-6 text-sm text-[var(--muted-text)]">Loading website connections...</div>
  }
  ...
  return (
    <div className="card rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
      <div className="px-5 py-4 border-b border-gray-200 dark:border-gray-700 flex items-center justify-between gap-3">
        <div>
          <h3 className="text-lg font-semibold text-theme">Website Connections</h3>
          <p className="text-sm text-[var(--muted-text)]">Manage websites that can send leads into the CRM.</p>
        </div>
        <button
          onClick={onCreate}
      ...
      {connections.length === 0 ? (
        <div className="p-8 text-center text-[var(--muted-text)]">
          ...
          <p>No website connections yet.</p>
        </div>
      ) : (
              ...
              <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                <div className="space-y-2 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-3 text-xs text-[var(--muted-text)]">
                    <span>Key: <span className="font-mono text-theme">{connection.masked_key}</span></span>
                    <span>Prefix: <span className="font-mono text-theme">{connection.key_prefix}</span></span>
                    <span>Leads: <span className="text-theme font-medium">{connection.leads_count || 0}</span></span>
                    <span>Requests: <span className="text-theme font-medium">{connection.requests_count || 0}</span></span>
                    {connection.source?.name ? <span>Source: <span className="text-theme font-medium">{connection.source.name}</span></span> : null}
                    {connection.campaign?.name ? <span>Campaign: <span className="text-theme font-medium">{connection.campaign.name}</span></span> : null}
                  </div>
                </div>
