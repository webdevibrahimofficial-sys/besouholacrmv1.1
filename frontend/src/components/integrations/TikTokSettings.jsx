...
export default function TikTokSettings({ onClose }) {
  const { t } = useTranslation()
  const [settings, setSettings] = useState(() => tiktokService.loadSettings())
  const [testPayload, setTestPayload] = useState(null)
  const [serverAck, setServerAck] = useState(null)

  useEffect(() => {
    tiktokService.saveSettings(settings)
  }, [settings])

  const status = useMemo(() => ({
    pixel: !!settings.pixelCode,
  ...
  const simulateEvent = () => {
    setTestPayload(tiktokService.simulateEvent(settings))
  }

  const sendTestEvent = async () => {
    try {
      const payload = testPayload || tiktokService.simulateEvent(settings)
      const j = await tiktokService.sendTestEvent(payload)
      setServerAck(j)
    } catch (e) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 border-b border-gray-200 dark:border-gray-800 pb-4">
        <div>
          <h2 className="text-xl font-bold">{t('TikTok Ads Settings')}</h2>
          <div className="flex items-center gap-2 mt-2 overflow-x-auto no-scrollbar">
            <Badge ok={status.pixel} label={t('Pixel')} />
            <Badge ok={status.api} label={t('Events API')} />
          </div>
        </div>
      ...
      <div className="card glass-card p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium opacity-70">{t('Pixel Code')}</label>
            <input className="input" value={settings.pixelCode || ''} onChange={(e)=>handleChange('pixelCode', e.target.value)} placeholder="e.g., C387..." />
          </div>
          <div className="flex flex-col gap-1">
            <label className="text-xs font-medium opacity-70">{t('Access Token')}</label>
            <input className="input" type="password" value={settings.accessToken || ''} onChange={(e)=>handleChange('accessToken', e.target.value)} placeholder="e.g., secret_..." />
          </div>
        </div>
      </div>
      ...
      <div className="card glass-card p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
           <button className="btn btn-primary btn-sm" onClick={simulateEvent}>{t('Simulate Web Event')}</button>
           <button className="btn btn-outline btn-sm" onClick={sendTestEvent}>{t('Send to TikTok API')}</button>
        </div>
        {testPayload && (
