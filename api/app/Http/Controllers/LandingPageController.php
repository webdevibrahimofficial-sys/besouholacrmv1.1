...
class LandingPageController extends Controller
{
    ...
    private function makeBaseLandingSlug(string $title): string
    {
        $title = mb_strtolower(trim($title), 'UTF-8');
        $slug = preg_replace('/[^\p{L}\p{N}]+/u', '-', $title) ?? '';
        $slug = trim($slug, '-');

        return $slug !== '' ? $slug : 'landing-page';
    }

    private function makeUniqueLandingSlug(string $title, ?int $ignoreId = null): string
    {
        $base = $this->makeBaseLandingSlug($title);
        $slug = $base;
        $counter = 2;

        while (true) {
            $query = LandingPage::withoutGlobalScope('tenant')->where('slug', $slug);
    ...
    private function exclusiveLeadContextRules(string $attribute, string $otherA, string $otherB): \Closure
    {
        return function ($attr, $value, $fail) use ($attribute, $otherA, $otherB) {
            if (empty($value)) {
                return;
            }

            $request = request();
            $hasOtherA = $request->filled($otherA);
            $hasOtherB = $request->filled($otherB);

            if ($hasOtherA || $hasOtherB) {
                $fail(sprintf(
                    'Only one of %s, %s, or %s may be provided.',
                    $attribute,
                    $otherA,
                    $otherB
                ));
        ...
        try {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'lead_project_id' => array_values(array_filter([
                    ...
                    $this->exclusiveLeadContextRules('lead_project_id', 'lead_unit_id', 'lead_item_id'),
                ...
                'lead_unit_id' => array_values(array_filter([
                    ...
                    $this->exclusiveLeadContextRules('lead_unit_id', 'lead_project_id', 'lead_item_id'),
                ...
                'lead_item_id' => array_values(array_filter([
                    ...
                    $this->exclusiveLeadContextRules('lead_item_id', 'lead_project_id', 'lead_unit_id'),
            ...
            // Generate Slug
            $data['slug'] = $this->makeUniqueLandingSlug((string) $request->title);
